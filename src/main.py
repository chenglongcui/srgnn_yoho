...
for epoch in range(opt.epoch):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        _, loss, _ = model.train(fetches, targets, item, adj_in, adj_out, alias, mask)
        loss_.append(loss)
    loss = np.mean(loss_)
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        scores, test_loss = model.pred([model.score_test, model.loss_test], targets, item, adj_in, adj_out, alias, mask)
