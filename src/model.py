...
class Model(object):
    ...
    def forward(self, re_embedding, train=True):
        if train:
            ...
            last_h = tf.gather_nd(re_embedding, tf.stack([tf.range(self.batch_size), last_id], axis=1))
            seq_h = tf.stack(
                [tf.nn.embedding_lookup(re_embedding[i], self.alias[i]) for i in range(self.batch_size)],
                axis=0)  # batch_size*T*d
            last = tf.matmul(last_h, self.nasr_w1)
            seq = tf.matmul(tf.reshape(seq_h, [-1, self.out_size]), self.nasr_w2)
            last = tf.reshape(last, [self.batch_size, 1, -1])
            ...
            if not self.nonhybrid:
                ...
                logits = tf.matmul(ma, b, transpose_b=True)
            self.logits = logits
            self.arg_sort = tf.nn.top_k(logits, 10)[1]
            # print(self.arg_sort)
            return self.logits, self.arg_sort

    def train(self, fetches, tar, item, adj_in, adj_out, alias, mask):
    ...
    def pred(self, fetches, item, adj_in, adj_out, alias, mask):
        return self.sess.run(fetches, feed_dict={self.item: item, self.adj_in: adj_in,
                                                 self.adj_out: adj_out, self.alias: alias, self.mask: mask})
...
class GGNN(Model):
    def __init__(self, hidden_size=100, out_size=100, batch_size=300, n_node=None,
        ...
        with tf.variable_scope('ggnn_model', reuse=True):
            self.logit, self.arg_sort = self.forward(self.ggnn(), train=False)
    ...
    def ggnn(self):
        ...
        # drop_cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=0.8)
        with tf.variable_scope('gru'):
            for i in range(self.step):
                ...
                state_output, fin_state = tf.nn.dynamic_rnn(cell,
                                                            tf.expand_dims(tf.reshape(av, [-1, 2 * self.out_size]),
                                                                           axis=1),
